...
ok(!$r->success, "Object registration correctly failed")
  or diag("Shouldn't be possible to register Object that inherits from primitive" . $r->message);
...
my %Namespace = ( namespaceType => 'RubbishNamespace',
...
my %RegSmpl = ( serviceName  => "1myfirstservice",
...
ok(!$r->success, "Service registration correctly failed with number as first character in serviceName")
  or diag("Service registration should have failed with numerical first character in serviceName: " . $r->message) ;
...
$r = $C->registerService(%RegSmpl);
ok($r->success, "Service registration correctly succeeded with a '_' as character in serviceName")
or diag("Service registration failed on an underscore in the service name (underscore is valid)" . $r->message) ;
# now get rid of it
$r = $C->deregisterService( serviceName  => 'myf_irstservice',
			    authURI      => 'test.suite.com' );
...
$RegSmpl{secondary} = { parametername1 => { datatype => 'INTEGER',# Break parameter datatype
					     description => "some parameter here",
					    default => 0,
					    max => 10,
					    min => -10,
...
$RegSmpl{secondary} = { parametername1 => { datatype => 'Integer', # Fix parameter datatype
				     description => "some parameter here",
					    default => 0,
					    max => 10,
					    min => -10,
...
$RegSmpl{input} = [ ['',  [Object => ['RubbishNamespace']]] ]; # Break input (no articleName)
...
$RegSmpl{input} = [ ['my!articlename',  [Object => ['RubbishNamespace']]] ]; # Break input (invalid articleName)
$r = $C->registerService(%RegSmpl);
ok(!$r->success, "Service registration correctly failed testing an invalid articleName")
  or diag("Service registration was supposed to fail due to an invalid articleName (contained a !) on input, but didn't: " . $r->message) ;

# Invalid input articlename - contains a ~
$RegSmpl{input} = [ ['myarticlename~',  [Object => ['RubbishNamespace']]] ]; # Break input (invalid articleName)
$r = $C->registerService(%RegSmpl);
ok(!$r->success, "Service registration correctly failed testing an invalid articleName")
  or diag("Service registration was supposed to fail due to an invalid articleName (contained a ~) on input, but didn't: " . $r->message) ;
...
$RegSmpl{input} = [ ['articleNameMultiSimples', [ [Object => ['RubbishNamespace'],
					       String => ['RubbishNamespace']
					      ]
					    ]
		] # Simple
	      ];
$RegSmpl{output} = [ ['articleNameSimpleSingle', [String => ['RubbishNamespace']]] ]; # Simple
...
my %boolSimpl = ( serviceName  => "mySecondaryTestservice",
                serviceType  => "Retrieval",
                authURI      => "test.suite.com",
                contactEmail => 'your@mail.address',
...
ok(!$r->success, "Service registration correctly failed because of the parameter type BOOLEAN in the secondary input.")
or diag("Service registration succeeded on a parameter type of BOOLEAN (should fail) " . $r->message) ;
# fix the parameter type
$boolSimpl{secondary} = { parametername1 => { datatype => 'Boolean', # Fix parameter datatype
                                     description => "some parameter here",
                                            default => 'false'
                                          }
                      };
# register it now
$r = $C->registerService(%boolSimpl);
ok($r->success, "Service registration correctly succeeded with secondary parameter type 'Boolean'.")
or diag("Service registration incorrectly failed on a secondary parameter type of Boolean (should not fail) " . $r->message) ;
# now get rid of it
$r = $C->deregisterService( serviceName  => 'mySecondaryTestservice',
                            authURI      => 'test.suite.com' );
################# SERVICE RETRIEVAL ##################
...
my ($si, $ri) = $C->findService( serviceName  => $RegSmpl{serviceName},
...
is(scalar(@ins), scalar @{$RegSmpl{input}}) or diag("incorrect number of inputs in service instance");
is(scalar(@outs), scalar @{$RegSmpl{output}}) or diag("incorrect number of outputs in service instance");
is(scalar(@secs), scalar keys %{$RegSmpl{secondary}}) or diag("incorrect number of secondary in service instance");
...
my $ns = shift @ns;
is($ns, $RegSmpl{input}->[0]->[1]->[1]->[0]) or diag("simple input reporting wrong namespace");

is($out->objectType,  $RegSmpl{output}->[0]->[1]->[0]) or diag("simple output type reported incorrectly");
is($out->articleName, $RegSmpl{output}->[0]->[0]) or diag("simple output article name reported incorrectly");
...
is($sec->default, $Reg_sec->{default}) or diag("secondary article reporting wrong default");
...
is($sec->description,     $Reg_sec->{description}) or diag("secondary article reporting wrong description");
...
for my $e (@enum) {
  ok(grep(/$e/, @{$Reg_sec->{enum}}), "Extra enum")
    or diag("Value '$e' returned by SecondaryArticle->{enum} but not specified in registration.");
}

######################       SERVICE WITH COLLECTIONS       #################
# Now register a second service, this time taking Collections for input and output.
my %RegColl = ( serviceName  => "mysecondservice",
		serviceType  => "Retrieval",
		authURI      => "test.suite.com",
		contactEmail => 'your@mail.address',
		...
		secondary    => { parametername1 => { datatype => 'Integer',
						      default => 0,
						      max => 10,
						      min => -10,
						      enum => [-10, 10, 0]}
...
($si, $r) = $C->findService( serviceName  => $RegColl{serviceName},
...
$SI = shift @$si;
#print STDERR "$SI";
isa_ok($SI, 'MOBY::Client::ServiceInstance') or diag("findService collections didn't return a MOBY::Client::ServiceInstance");
...
is($SI->type,          $RegColl{serviceType}) or diag("service type incorrect (collections test)");
is($SI->description,   $RegColl{description}) or diag("service description wrong (collections test)");
is($SI->URL,           $RegColl{URL}) or diag("service URL incorrect (collections test)");
is($SI->contactEmail,  $RegColl{contactEmail}) or diag("contact email incorrect (collections test)");
is($SI->category,      $RegColl{category}) or diag("service category incorrect (collections test)");

@ins = @{$SI->input};
@outs = @{$SI->output};
@secs = @{$SI->secondary};
is(scalar(@ins),  scalar @{$RegColl{input}}) or diag("incorrect number of inputs in service instance (collections test)");
is(scalar(@outs), scalar @{$RegColl{output}}) or diag("incorrect number of outputs in service instance (collections test)");
is(scalar(@secs), scalar keys %{$RegColl{secondary}}) or diag("incorrect number of secondary in service instance (collections test)");
...
isa_ok($in, 'MOBY::Client::CollectionArticle')
  or diag("->inputs did not return a MOBY::Client::Collection input object or client parser failed for MOBY::Client::CollectionArticle");

isa_ok($out, 'MOBY::Client::CollectionArticle')
  or diag("->outputs did not return a MOBY::Client::CollectionArticle output object or client parser failed for MOBY::Client::CollectionArticle");
...
$ns = shift @ns;
is($ns, $reg_in->[1]->[0]) or diag("simple input reporting wrong namespace (collections test)");
...
isa_ok($r->{$RegSmpl{authURI}}, 'ARRAY') or diag("retrieveServiceNames didn't return a hasref of arrayrefs");
...
isa_ok($rel{'urn:lsid:biomoby.org:objectrelation:isa'}, 'ARRAY',
       "Object reporting correct ISA as arrayref")
  or diag("Object definition did not report ISA arrayref");

isa_ok($rel{'urn:lsid:biomoby.org:objectrelation:hasa'}, 'ARRAY',
       "Object reporting correct HASA as arrayref")
